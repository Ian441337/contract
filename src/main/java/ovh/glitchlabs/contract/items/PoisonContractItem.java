...
public class PoisonContractItem extends Item {
    public PoisonContractItem(Properties properties) {
        super(properties);
    }
    ...
    public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand hand) {
        ...
        if (!level.isClientSide) {
            if (!isBound(stack)) {
                ...
                player.sendSystemMessage(Component.literal("Poison Contract bound to: " + player.getName().getString())
                ...
                if (boundUUID != null) {
                    ...
                    if (boundPlayer != null && boundPlayer != player) {
                        boundPlayer.addEffect(new MobEffectInstance(MobEffects.POISON, 200, 0)); // 10 seconds
                        stack.setCount(0);
                        
                        player.sendSystemMessage(Component.literal("Poisoned " + boundPlayer.getName().getString())
                                .withStyle(ChatFormatting.DARK_GREEN));
                        boundPlayer.sendSystemMessage(Component.literal("You have been poisoned!")
                                .withStyle(ChatFormatting.DARK_GREEN));
                    }
                }
            }
        ...
        if (customData != null) {
            ...
            if (tag.contains("BoundPlayer")) {
                ...
                tooltip.add(Component.literal("Bound to: " + playerName)
                        .withStyle(ChatFormatting.DARK_GREEN));
                tooltip.add(Component.literal("Right-click to poison the bound player")
