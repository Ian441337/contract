...
public class HungerContractItem extends Item {
    public HungerContractItem(Properties properties) {
        super(properties);
    }
    ...
    public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand hand) {
        ...
        if (!level.isClientSide) {
            if (!isBound(stack)) {
                ...
                player.sendSystemMessage(Component.literal("Hunger Contract bound to: " + player.getName().getString())
                ...
                if (boundUUID != null) {
                    ...
                    if (boundPlayer != null && boundPlayer != player) {
                        boundPlayer.addEffect(new MobEffectInstance(MobEffects.HUNGER, 600, 2)); // 30 seconds
                        stack.setCount(0);
                        
                        player.sendSystemMessage(Component.literal("Made " + boundPlayer.getName().getString() + " hungry!")
                                .withStyle(ChatFormatting.GOLD));
                        boundPlayer.sendSystemMessage(Component.literal("You are starving!")
                                .withStyle(ChatFormatting.GOLD));
                    }
                }
            }
        ...
        if (customData != null) {
            ...
            if (tag.contains("BoundPlayer")) {
                ...
                tooltip.add(Component.literal("Bound to: " + playerName)
                        .withStyle(ChatFormatting.GOLD));
                tooltip.add(Component.literal("Right-click to make the bound player hungry")
